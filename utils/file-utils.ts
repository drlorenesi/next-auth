...
export function parseFileContent(content: string) {
  if (!content || typeof content !== "string") {
    throw new Error(
      "Invalid file content: File appears to be empty or not a text file"
    );
  }
  ...
  if (lines.length < 3) {
    throw new Error(
      "Invalid file format: File must contain at least metadata and header lines"
    );
  }
  ...
  const headerLineIndex = lines.findIndex(
    (line) => line.includes("Date") && line.includes("Time")
  );

  if (headerLineIndex === -1) {
    throw new Error(
      "Invalid file format: Could not find header line with 'Date' and 'Time' columns"
    );
  }
...
export function readAndParseFile(file: File): Promise<ParsedData> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (err instanceof Error) {
          ...
          reject(
            new Error(
              "Failed to parse file. Please ensure it's in the correct format."
            )
          );
        }
      }
    ...
    try {
      ...
      reject(
        new Error(
          "Failed to read file. The file may be corrupted or in an unsupported format."
        )
      );
    }
  });
 ...
 */
export function findQFColumnIndices(
  headers: string[]
): { qfIndex: number; dataIndex: number }[] {
  if (!headers || !Array.isArray(headers)) {
    return [];
  }

  return headers.reduce<{ qfIndex: number; dataIndex: number }[]>(
    (acc, header, index) => {
      if (header === "QF") {
